...
from CNN_Custom_Model import BetterCNN
...
if use_EfficientNet_B4:
    ...
    # Save result in different location
    SAVE_RESULTS_PATH = os.path.join(SAVE_RESULTS_PATH, 'pretrained_model')
else:
    # use our custom CNN model
    image_width = 224
    in_channels = 3
    model = BetterCNN(image_width, in_channels, num_classes)
    # Save result in different location
    SAVE_RESULTS_PATH = os.path.join(SAVE_RESULTS_PATH, 'our_model')
model.to(DEVICE)
